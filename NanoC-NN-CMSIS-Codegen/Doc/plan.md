# NanoC-NN-CMSIS-Codegen 技术路线与里程碑

## 1. 子项目目标

NanoC-NN-CMSIS-Codegen 是 **CMSIS-NN ONNX 端侧代码生成器** 中的代码生成模块，目标是读取 `NanoC-NN-ONNX-Converter` 输出的中间表示和权重资料，生成基于 Arm CMSIS-NN 的 C 推理工程。

本子项目的限定场景是 Arm 架构嵌入式低算力平台，典型目标包括 STM32、GD32 等 Cortex-M MCU。生成代码应服务 MCU 固件开发：资源有限、固定 shape、batch size 1、无运行期动态内存、可交叉编译、可审查、可插入裸机或 RTOS 工程。

本子项目不重新解析 ONNX 文件，也不自研完整神经网络算子库。它的核心职责是：
...
```text
NanoC-NN-CMSIS-Codegen/third_party/CMSIS-NN/
```

本地源码来自 `ARM-software/CMSIS-NN` 的 `main` 分支 tarball，保留源码与 `LICENSE`，但不包含嵌套 `.git` 历史。
...
`NanoC-NN-CMSIS-Codegen` 的唯一上游必须是 `NanoC-NN-ONNX-Converter`。
...
- `Doc/`：技术路线、里程碑和 CMSIS-NN 映射设计文档。
- `nanoc_cmsis_codegen/`：代码生成器 Python 包。
- `templates/`：C 工程模板。初期使用简单占位模板，不引入模板框架。
- `tests/`：schema 校验、算子映射、命名、内存规划和生成结果测试。
...
- 建立 Python 包和 `pyproject.toml`。
- 实现 `python -m nanoc_cmsis_codegen --input --out` 的入口。
