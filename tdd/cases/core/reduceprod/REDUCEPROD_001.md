...
内部探索：沿已支持的 reduce family 扩展乘积归约能力。
...
```text
Input -> QuantizeLinear -> DequantizeLinear -> ReduceProd -> QuantizeLinear -> DequantizeLinear -> Output
```
...
|------|-------|------------|------|
| input | `0.05` | `0` | 输入量化 |
| output | `0.01` | `0` | 输出乘积量化 |

## 预期结果

- **codegen status**: `ok`
- **若 ok**: 编译通过、运行不崩溃
- **关键验证点**: 生成 C 应执行乘积归约，而不是复用 sum/mean 语义

## 数值验收

- **是否需要**: `yes`
- **数据集**: `tdd/fixtures/datasets/reduceprod_qdq_smoke/dataset.json`
